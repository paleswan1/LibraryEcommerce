    ...
    public class DashboardService(IGenericRepository genericRepository) : IDashboardService
    {
        ...
        public async Task<GetLibraryDashboardOverviewDto> GetLibraryDashboardOverviewAsync(int period)
        {
            ...
            var weekfromDate = DateTime.UtcNow.AddDays(-7 * 4); // Last 4 weeks

    // Fetch only necessary data to minimize memory load
            var rawWeeklyOrders = await genericRepository
                .Get<Order>(o => o.CreatedAt >= weekfromDate)
                .Select(o => new { o.Id, o.CreatedAt }) // Reduce payload
                .ToListAsync();

    // Group in-memory by ISO week
            var weeklyOrders = rawWeeklyOrders
                .GroupBy(o =>
                {
                    var weekStart = o.CreatedAt.StartOfWeek();
                    return new
                    {
                        Year = weekStart.Year,
                ...
                .Select(g => new GetWeeklyOrderDto
                {
                    Year = g.Key.Year,
                    Week = g.Key.Week,
                    TotalOrders = g.Count()
        ...
        public Task<List<GetWeeklyOrderDto>> GetWeeklyOrdersAsync(int numberOfWeeks)
        {
